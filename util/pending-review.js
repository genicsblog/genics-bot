...
async function getNewFiles() {
	...
	for (let i = 0; i < response.data.length; i++) {
		...
		if (data['drafts'].includes(file.name.replace('.md', '').toLowerCase()))
			continue
		...
		if (resp.data.name != 'test.md') {
			...
			if (attributes.original === null || attributes.original === undefined) {
				const permalink =
					...
					resp.data.name.replace('.md', '').toLowerCase()
...
function sendMessage(newFiles, client, notify) {
	if (newFiles.length == 0) {
		if (notify)
			client.channels.cache
				.get(process.env.REVIEW_CHANNEL_ID)
				.send('No articles to review :D')
		return
	}

	var message =
		'<@&935778876448190574> Here are some new articles to review!\n\n'
	...
	fs.writeFile('./temp.txt', jsonString, err => {
		if (err) {
			...
			client.channels.cache
				.get(process.env.JSON_CHANNEL_ID)
				.send({ files: ['./temp.txt'] })
		}
	})
}
...
module.exports = async (client, notify) => {
	client.channels.cache
		...
		.send('Retreiving drafts that are pending review...')
