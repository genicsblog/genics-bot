...
module.exports = async client => {
	client.channels.cache
		...
		.send('Time to log analytics data...')
...
⦁ A total of **${data[0].value} viewers** visited genics, out of which **${
		data[1].value
	}** were new to the site!

⦁ There were **${
		data[3].value
	} sessions** and the average session duration was **${prettySeconds(
		parseFloat(data[4].value)
	)}**!

⦁ The bounce rate was **${(parseFloat(data[5].value) * 100.0).toFixed(2)}%**
